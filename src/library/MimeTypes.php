...
class MimeTypes
{
    ...
    public function getExtension(string $type, bool $advance = false): string
    {
        $ext = $this->find($this->extensions, 'extensions', $type);
        if ('' === $ext && $advance) {
            $tmp = explode('/', $type);
            if (isset($tmp[1])) {
                $subtype = $tmp[1];
                foreach ($this->types as $t) {
                    $ext = $this->find($this->extensions, 'extensions', $t . '/' . $subtype);
                    if ($ext) {
                        return $ext;
                    }
                }
    ...
    public function getMime(string $ext): string
    {
        return $this->find($this->mimes, 'mimes', $ext);
    }

    private function find(array &$data, string $type, $key): string
    {
        $key = strtolower($key);
        if (isset(self::$result[$type][$key])) {
            return self::$result[$type][$key];
        }
        if (isset($data[$key])) {
            self::$result[$type][$key] = $data[$key][0];
